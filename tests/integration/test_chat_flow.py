...
# Test database URL (use shared in-memory SQLite for speed)
TEST_DATABASE_URL = "sqlite+aiosqlite:///:memory:?cache=shared&uri=true"
...
class TestChatFlowEndToEnd:
    ...
    async def test_conversation_history_persists(
        ...
        async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
            ...
            # API returns newest first, so last message should be user's "Hello Marcus"
            user_messages = [msg for msg in history if msg["role"] == "user"]
            assert len(user_messages) >= 2
            assert any("Hello Marcus" in msg["content"] for msg in user_messages)
...
class TestDatabaseIntegration:
    ...
    async def test_message_persistence(self, test_service):
        ...
        assert history[0].role == "user"  # Oldest first
